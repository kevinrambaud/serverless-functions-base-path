 ...
 *
 * Note: only properties with primitive value will be copied,
...
function validatePath(basePath, prevFunctions, currFunctions) {
  Object.keys(prevFunctions).forEach(functionName => {
    const expectedPath = path.posix.join(
      basePath,
      prevFunctions[functionName].handler
    );
...
describe('BasePathPlugin', () => {
  ...
  it('should skip a function if it has an image specified', () => {
    const mock = copyObj(this.serverlessMock);
    mock.service.functions.dockerized = {
      image: {
        name: 'app'
      }
    };

    const Plugin = new BasePathPlugin(mock, this.optionsMock);
    const previousFunctions = copyObj(Plugin.serverless.service.functions);
    ...
    expect(previousFunctions.dockerized).to.be.deep.equal(
      currentFunctions.dockerized
    );
  });
});
